...
function Parametrler() {
  ...
  const handleOlcuElave = async () => {
    ...
    try {
      ...
      await addOlcu({ id: yeniId, ad: formattedOlcuAd });
      setYeniOlcuAd('');
    } finally {
      setLoading(false);
    }
          ...
          <div className="bg-white rounded-xl shadow-md p-6 mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-2 md:gap-4 mb-4">
              ...
              <button
                ...
                disabled={loading}
                className="w-full sm:w-auto bg-blue-600 text-white px-4 py-2 text-sm md:px-6 md:py-2 md:text-base rounded-lg hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-1 md:gap-2"
              >
                {loading ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-plus"></i>} Əlavə Et
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {data.olculer.map((o) => (
